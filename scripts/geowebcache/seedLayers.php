<?php
  ...
  $a = array("lvg_residence");
  
  for($i=1;$i<2;$i++) {
      foreach($a as $aa) {
	$s = explode("_",$aa);
	$style = "la_".substr($s[1],0,4)."_".$i;
        $curl = 'curl -v -u '.$geoserver_username.':'.$geoserver_password.' -XPOST -H "Content-type: text/xml" -d "<seedRequest><name>stko:'.$aa.'</name><srs><number>900913</number></srs><zoomStart>10</zoomStart><zoomStop>11</zoomStop><format>image/png</format><type>reseed</type><threadCount>1</threadCount><parameters><entry><string>STYLES</string><string>'.$style.'</string></entry></parameters></seedRequest>"  "'.$geoserver_url.'/geoserver/gwc/rest/seed/stko:'.$aa.'.xml"';
	exec($curl);
	sleep(30);
      }
  }
